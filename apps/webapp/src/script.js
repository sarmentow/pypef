...
canvas.addEventListener('mousedown', (e) => {
  if (mode === 'drawing' && !isDrawing) {
    isDrawing = true;
    lastVertex = new Ponto(e.offsetX, e.offsetY);
    ...
    for (let i = vertices.length - 1; i >= 0; i--) {
      ...
      if (Math.abs(vertex.x - e.offsetX) < 5 && Math.abs(vertex.y - e.offsetY) < 5) {
        selectedVertex = i;
        document.getElementById('selected-vertex').innerHTML = `Selected vertex: x = ${vertex.x}, y = ${vertex.y}`;
        ...
        document.getElementById('update-vertex').addEventListener('click', () => {
          ...
          const newVertex = new Ponto(newX, newY);
          newVertex.Vinculo = {
            f: {
...
addCargaBtn.addEventListener('click', () => {
  const fx = parseInt(document.getElementById('fx').value);
  const fy = parseInt(document.getElementById('fy').value);
  const fz = parseInt(document.getElementById('fz').value);
...
document.getElementById('preset').addEventListener('change', (e) => {
  if (e.target.value === '0') {
    ...
    const inicio = new Ponto(0, 0);
    inicio.Vinculo = {
      f: new Vector3(false, false, false),
      m: new Vector3(true, true, true) // Engaste
    };

    const fim = new Ponto(100, 0);
    fim.Vinculo = {
      f: new Vector3(false, false, false),
